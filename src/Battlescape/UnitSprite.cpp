 ...
 */
UnitSprite::UnitSprite(int width, int height, int x, int y, bool helmet, int bpp) : Surface(width, height, x, y, bpp), _unit(0), _itemA(0), _itemB(0), _unitSurface(0), _itemSurfaceA(0), _itemSurfaceB(0), _part(0), _animationFrame(0), _drawingRoutine(0), _helmet(helmet), _color(0), _colorSize(0)
{
}
...
void UnitSprite::drawRoutine0()
{
	...
	if (_unit->getStatus() == STATUS_COLLAPSING)
	{
		...
		if (torso->getSurface()->format->BitsPerPixel == 8)
		{
