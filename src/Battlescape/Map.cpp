 ...
 */
Map::Map(Game *game, int width, int height, int x, int y, int visibleMapHeight, int bpp) : InteractiveSurface(width, height, x, y, bpp), _game(game), _arrow(0), _selectorX(0), _selectorY(0), _mouseX(0), _mouseY(0), _cursorType(CT_NORMAL), _cursorSize(1), _animFrame(0), _projectile(0), _projectileInFOV(false), _explosionInFOV(false), _launch(false), _visibleMapHeight(visibleMapHeight), _unitDying(false), _smoothingEngaged(false), _flashScreen(false)
{
...
void Map::cacheUnit(BattleUnit *unit)
{
	...
	UnitSprite *unitSprite = new UnitSprite(_spriteWidth * 2, _spriteHeight, 0, 0, _save->getDepth() != 0, bpp);
	...
	if (invalid)
	{
		...
		for (int i = 0; i < numOfParts; i++)
		{
			...
			{
				cache = new Surface(_spriteWidth * 2, _spriteHeight, 0, 0, bpp);
